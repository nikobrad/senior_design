...
void updateMotorRotation(uint8 motorIndex)
{
    ...
    tmp = tmp + (motorDat[motorIndex].index * STEP_CONSTANT);
    motorDat[motorIndex].counter = tmp;
}

void linearConv(uint8 motorIndex) // milli-inche
{
    motorDat[motorIndex].lineLength = (motorDat[motorIndex].counter * STEP_SIZE * PI * SPOOL_DIAMETER) / 360; // milli-inches
}
...
void calcPayloadCenter(uint8 motorIndex)
{
    ...
    for(i = 0;i < 2;i = i + 1)
    {
        float sideA = (float)motorDat[motorIndex % 4].lineLength / 1000.0; // Scale triangle sides to actual size for float motorIndexth
        float sideB = (float)motorDat[(motorIndex + 1) % 4].lineLength / 1000.0;
        float sideC = (float)FRAME_DIAMETER / 1414.0;
        float tmp = ((sideA*sideA) + (sideC*sideC) - (sideB*sideB)) / (2*sideA*sideC); // Law of Cosines; find angle theta of line attached to motor motorIndex
        ...
        PAYLOAD_CENTER[(motorIndex + 1) % 2] = ret; // For even motorIndex, calculates py and stores in PAYLOAD_CENTER[1]; for odd motorIndex, calculates px and stores in PAYLOAD_CENTER[0]
        char prt[16]; // Print findings to UART terminal for verification
        sprintf(prt,"Motor %d: Coordinate %d\n\r",(motorIndex % 4),ret);
        UART_UartPutString(prt);
        motorIndex = motorIndex + 1; // Increment motorIndex to do it all over again; find other part of coordinates, verify calculations, etc
    }
    for(i = 0;i < 2;i = i + 1) // Same code, but doesn't change any values; just calculating same values from other two line lengths for confirmotorIndextion
    {
        float sideA = (float)motorDat[motorIndex % 4].lineLength / 1000.0;
        float sideB = (float)motorDat[(motorIndex + 1) % 4].lineLength / 1000.0;
        float sideC = (float)FRAME_DIAMETER / 1414.0;
        ...
        tmp = tmp * 1000;
        int ret = (int) tmp;
        //PAYLOAD_CENTER[(motorIndex + 1) % 2] = ret;
        char prt[16];
        sprintf(prt,"Motor %d: Coordinate %d\n\r",(motorIndex % 4),ret);
        UART_UartPutString(prt);
